    ...
    return (
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {visibleCaseStudies.map((card) => (
                <Link key={card.id} to={`/case-studies/${card.slug}`} aria-label="case study card">
                    <div className="h-[432px] group p-3 rounded-xl border border-[#EDEDED] transition-all duration-400 hover:shadow-[0_0_3px_6px_#EAF4FF] hover:border-[#0160C9] flex flex-col items-start justify-between gap-4">
                        ...
                        <div className="flex flex-col items-start gap-2 w-full">
                            ...
                            <Link to={`/case-studies/${card.slug}`} className="mt-1 flex items-center gap-1 text-sm font-semibold leading-5 text-[#0160C9]" aria-label="read more btn">
                                Read more
